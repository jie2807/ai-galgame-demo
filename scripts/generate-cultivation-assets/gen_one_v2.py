#!/usr/bin/env python3
"""单张图片生成脚本 v2：更长间隔，更少重试
用法 python gen_one_v2.py <id> <type>
        ...
        try:
            ...
            if result.get("status") == "success" and result.get("success_list"):
                ...
                if attempt < MAX_RETRY:
                    print(f"  wait {RETRY_WAIT}s before retry...")
...
async def main():
    if len(sys.argv) < 3:
        print("Usage: python gen_one_v2.py <id> <portrait|scene>")
        sys.exit(1)
    item_id = sys.argv[1]
